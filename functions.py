...
def kmean_eda(data,seed=42, k_start=3,k_end=20,kint=1,n_init=10,minibatch=False, batch_size=10,silhouette=False):
    """
    ...
    for ik in range(k_start,k_end+1,kint):
        ...
        np.random.seed(seed)
        if (minibatch):
            kmeans = MiniBatchKMeans(n_clusters=ik,batch_size=batch_size, init = 'k-means++',n_init=n_init)
            kmeans.fit(data)
            if(silhouette):
                silhouette_c.append(silhouette_score(data, kmeans.labels_))    
        else:
            kmeans = KMeans(n_clusters=ik, init = 'k-means++',n_init=n_init)
...
def batch_kmean_multiple(data,k=4,name_customer='CustomerID',random_seed=45,
                         init='k-means++',n_init=3,max_iter=100,batch_size=1000):
    """
    Clustering algorithm for mini-batch
    """
    ...
    # MiniBatchKMeans
    kmeans_batch = MiniBatchKMeans(n_clusters = k,init=init,max_iter=max_iter,
        batch_size = batch_size,n_init=n_init,random_state = random_seed)
    kmeans_batch.fit_predict(data_std)
    
    kmeans_batch_pred = list(kmeans_batch.labels_)
    kmeans_batch_pred = [a + 1 for a in kmeans_batch_pred]  
    kmeans_batch_pred = [f'cluster{str(a)}' for a in kmeans_batch_pred]   # convert to str
    
    # adding to dataframe
    data_=data.copy()

    data_.insert(0, name_customer, data_.index)
    data_['cluster'] = kmeans_batch_pred
    print(data_[['cluster']].value_counts())
    return kmeans_batch.cluster_centers_,data_[['cluster']].value_counts(normalize=True),data_.reset_index(drop=True)  
    
    #########################################################################         
                
def Flow_old_new_cluster(dfde1,dfde2,label1,label2,clm1_1='CustomerID', clm1_2='cluster',
                         clm2_1='CustomerID', clm2_2='cluster',print_=False,nlcuster=4):
    '''
    customer Flow of Clusters for Sankey diagram 
    ...
    for ilabl in label1:
        n=0
        customer_1=np.array(dfde1[clm1_1][(dfde1[clm1_2]==ilabl)]) 
        customer_1=np.unique(customer_1)
        for jlabl in label2:
            if (ilabl==jlabl and n==0):
                n=n+1
                customer_2=np.array(dfde2[clm2_1][(dfde2[clm2_2]==jlabl) &
                                           (dfde2[clm2_1].isin(customer_1))])
                customer_2_.append(len(np.unique(customer_2)))
                if(print_): print(ilabl,jlabl,len(np.unique(customer_2)))
            elif (ilabl!=jlabl):
                customer_2=np.array(dfde2[clm2_1][(dfde2[clm2_2]==jlabl) &
                                                  (dfde2[clm2_1].isin(customer_1))]) 
                customer_2_.append(len(np.unique(customer_2)))
    ...
    value=customer_2_            
    # Define source
    source=list(np.ravel(np.array([[j]*len(label2) for j in range(len(label1))])))
    
    # Define target
    target=list(np.ravel(np.array([[i for i in range (len(label1), (len(label1)+len(label2)))] 
                                   for j in range(len(label1))])))
    target_maj_vot=list(np.ravel(np.array([[i-4 for i in range (len(label1), (len(label1)+len(label2)))]
                                   for j in range(len(label1))])))

    dic_val={}
    for i in range(len(target_maj_vot)):
        dic_val[source[i],target_maj_vot[i]]=value[i]
    maj_vot=[]    
    for i in range(nlcuster):    
        val=[dic_val[(i,j)] for j in range(nlcuster)]
        maj_vot.append(f'cluster{val.index(max(val))+1}')   
    dup=([item for item, count in collections.Counter(maj_vot).items() if count > 1]) 
    #if (len(dup)==1):
    #    No_cluster=[i for i in label2 if i not in maj_vot]
    #    index=[i for i,j in enumerate (maj_vot) if j ==dup[0]][1]
    #    maj_vot[index]=No_cluster[0]
    if (len(dup)>=1): 
        raise ValueError (f'There are more than two missing clusters:{maj_vot}')        

    return source, target, value,maj_vot    